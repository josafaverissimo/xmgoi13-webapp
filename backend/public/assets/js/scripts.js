...
function notRefresh() {
    ...
    function appendLinksIfNotExists(links) {
        links.forEach(link => {
            const linkExist = [].some.call(
                document.head.querySelectorAll("link"),
                linkExisting => link.href === linkExisting.href
            )

            if(!linkExist) document.head.appendChild(link)
        })
    }

    function appendScriptsIfNotExists(scripts) {
        scripts.forEach(script => {
            const scriptExist = [].some.call(
                document.head.querySelectorAll("footer script"),
                scriptExisting => script.src === scriptExisting.src
            )
